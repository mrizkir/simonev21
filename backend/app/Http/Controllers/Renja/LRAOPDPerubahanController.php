...
class LRAOPDPerubahanController extends Controller 
{
  /**
  ...
  */
  ...
  {             
    ...
    $form_lra_opd = new FormLRAPerubahanOPDModel([], false);
    $data_lra = $form_lra_opd->getDataLRA($OrgID, $no_bulan, $tahun, 2);
    ...
    if(isset($tangkat_lra[1]))
    {
      ...
      foreach($tingkat_1 as $k1 => $v1)
      {
        ...
        $data[] = [
          'FormLRAPerubahanDetailID' => Uuid::uuid4()->toString(),
          'tingkat' => 1,
          'kode' => $k1,
          'nama_uraian' => $v1,
    ...
    return Response()->json([
      ...
      'message' => 'Fetch data lra opd perubahan berhasil diperoleh'
    ], 200);    
  }  
  public function printtoexcel (Request $request)
  {
    ...
    if (\DB::table('trRKA')->where('OrgID', $opd->OrgID)->where('EntryLvl', 1)->where('TA', $tahun)->count() > 0)
    {
      ...
      $report = new \App\Models\Renja\FormLRAPerubahanOPDModel($data_report);
      $generate_date = date('Y-m-d_H_m_s');
      return $report->download("lra_opd_perubahan_$generate_date.xlsx");
    }
    else
    {
      return Response()->json([
