...
class EvaluasiMurniRealisasiTWModel extends ReportModel
{   
  public function __construct($dataReport, $print = true)
  {
    ...
    if ($print)
    {
      $this->spreadsheet->getProperties()->setTitle("Evaluasi Realisasi Murni Per T.W Tahun ".$this->dataReport['tahun']);
      $this->spreadsheet->getProperties()->setSubject("Evaluasi Realisasi Murni Per T.W Tahun ".$this->dataReport['tahun']); 
      $this->print();             
    }        
  }    
  private function print()  
  {
    ...
    $sheet->setTitle('Eval Realisasi Murni TW');
    ...
    // Set title
    $row = 1;
    $sheet->setCellValue('A' . $row, 'EVALUASI REALISASI MURNI PER T.W');
    ...
    $row += 2;
    $row_header_2 = $row + 1;
    $sheet->mergeCells("A$row:A$row_header_2");
    $sheet->setCellValue("A$row", 'NO');
    
    $sheet->mergeCells("B$row:B$row_header_2");
    $sheet->setCellValue("B$row_header_2", 'KODE');
    
    $sheet->mergeCells("C$row:C$row_header_2");
    $sheet->setCellValue("C$row", 'NAMA OPD');
    
    $sheet->mergeCells("D$row:D$row_header_2");    
    $sheet->setCellValue("D$row", 'PAGU DANA (RP)');
    
    $sheet->mergeCells("E$row:E$row_header_2");    
    $sheet->setCellValue("E$row", 'BOBOT (%)');
    
    $sheet->mergeCells("F$row:G$row");    
    $sheet->setCellValue("F$row", 'FISIK');
    
    $sheet->setCellValue("F$row_header_2", 'TARGET(%)');
    $sheet->setCellValue("G$row_header_2", 'REALISASI(%)');
    
    $sheet->mergeCells("H$row:H$row_header_2");    
    $sheet->setCellValue("H$row", 'KATEGORI');
    
    $sheet->mergeCells("I$row_header_2");    
    $sheet->setCellValue("I$row", 'TARGET (RP)');
    $sheet->setCellValue("J$row", 'TARGET (%)');        
    $sheet->setCellValue("K$row", 'REALISASI KEUANGAN');
    $sheet->setCellValue("L$row", '% REALISASI KEUANGAN');
