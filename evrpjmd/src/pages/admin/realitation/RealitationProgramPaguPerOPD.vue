        ...
        <template v-slot:item="{ index, item }">
          ...
          <template v-if="item.pagu.length > 0">
            <template v-for="(pagu, i) in item.pagu" :key="pagu.RpjmdRealisasiIndikatorID">              
              <tr class="text-center border-thin">
                ...
                <td class="bg-lime-lighten-2 border-thin">{{ $filters.formatUang(pagu.target_7) }} / {{ $filters.formatUang(pagu.realisasi_7) }}</td>                
          ...
          <template v-else>
            <tr class="bg-green-lighten-5 border-thin">
              <td colspan="15" class="text-center border-thin">Belum ada realisasi pagu. Silahkan tambah</td>
            </tr>
          </template>
        </template>
      </v-data-table-server>
