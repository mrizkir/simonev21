		...
		<ModuleHeader>
			...
			<template v-slot:desc>
				<v-alert color="cyan" border="left" colored-border type="info">
					Ubah Uraian Rencana Kegiatan dan Anggaran (RKA) OPD / Unit Kerja APBD
					Perubahan
				</v-alert>
			</template>
		</ModuleHeader>
				...
				<v-col cols="12">
					<v-form ref="frmedituraian" v-model="form_valid" lazy-validation>
						<v-card class="mb-2">
							<v-card-text>
								...
								<v-text-field
									...
									:value="
										'[' +
											datakegiatan.kode_kegiatan +
											'] ' +
											datakegiatan.Nm_Kegiatan
									"
									:disabled="true"
								/>
								<v-text-field
									...
									filled
									:value="
										'[' + datauraian.kode_uraian + '] ' + datauraian.nama_uraian
									"
									:disabled="true"
								/>
								<v-text-field
						...
						<v-card>
								...
								>
									...
									>
										...
										<v-btn
											text
											color="primary"
											@click="menuTanggalKontrak = false"
										>
											Cancel
										</v-btn>
										<v-btn
											...
											color="primary"
											@click="
												$refs.menuTanggalKontrak.save(formdata.tgl_kontrak)
											"
										>
											OK
										</v-btn>
								...
								>
									...
									>
										...
										<v-btn
											text
											color="primary"
											@click="menuTanggalMulaiPelaksanaan = false"
										>
											Cancel
										</v-btn>
										<v-btn
											...
											color="primary"
											@click="
												$refs.menuTanggalMulaiPelaksanaan.save(
													formdata.tgl_mulai_pelaksanaan
												)
											"
										>
											OK
								...
								>
									...
									>
										...
										<v-btn
											text
											color="primary"
											@click="menuTanggalSelesaiPelaksanaan = false"
										>
											Cancel
										</v-btn>
										<v-btn
											...
											color="primary"
											@click="
												$refs.menuTanggalSelesaiPelaksanaan.save(
													formdata.tgl_selesai_pelaksanaan
												)
											"
										>
											OK
							...
							<v-card-actions>
								<v-spacer></v-spacer>
								<v-btn color="blue darken-1" text @click.stop="closeedituraian">
									TUTUP
								</v-btn>
								<v-btn
									color="blue darken-1"
	...
	export default {
		...
		created() {
			...
			if (Object.keys(this.datauraian).length > 1) {
				...
				this.initialize();
				if (
					this.formdata.SumberDanaID == null ||
					this.formdata.SumberDanaID == ""
				) {
					this.formdata.SumberDanaID = this.datakegiatan.SumberDanaID;
				}
				if (
					this.formdata.idlok != null &&
					this.formdata.idlok != "" &&
					this.formdata.idlok.length > 0
				) {
		...
		methods: {
			...
			updateuraian: async function() {
				if (this.$refs.frmedituraian.validate()) {
					...
					await this.$ajax
						.post(
							"/renja/rkaperubahan/updatedetailuraian/" + this.formdata.RKARincID,
							{
								_method: "PUT",
			...
			closeedituraian() {
				this.$router.push(
					"/renjaperubahan/rka/realisasi/" + this.formdata.RKARincID
				);
			},
		},
		watch: {
