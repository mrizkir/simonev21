					...
					>
						...
						<template v-slot:item.actions="{ item }">
							<v-tooltip bottom>
								...
								<span>Ubah Jenis PEMBANGUNAN</span>
							</v-tooltip>
							<v-tooltip bottom>
								<template v-slot:activator="{ on, attrs }">
									<v-icon
								...
								<span>Hapus Jenis PEMBANGUNAN</span>
							</v-tooltip>
						</template>
						<template v-slot:expanded-item="{ headers, item }">
							<td :colspan="headers.length" class="text-center">
								<strong>ID:</strong>
								{{ item.JenisPembangunanID }}
			...
			save() {
				if (this.$refs.frmdata.validate()) {
					...
					if (this.editedIndex > -1) {
						this.$ajax
							.post(
								"/dmaster/jenispembangunan/" + this.formdata.JenisPembangunanID,
								{
									_method: "PUT",
						...
						this.$ajax
							.post(
								"/dmaster/jenispembangunan/store",
								{
									NamaJenis: this.formdata.NamaJenis,
									Descr: "",
				...
				this.$root.$confirm
					...
					.then(confirm => {
						if (confirm) {
							...
							this.$ajax
								.post(
									"/dmaster/jenispembangunan/" + item.JenisPembangunanID,
									{
										_method: "DELETE",
									},
									{
