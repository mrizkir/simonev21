								...
								<v-dialog v-model="dialogcopyfrm" max-width="500px" persistent>
									...
									>
										<v-card>
											<v-card-title>
												<span class="headline">
													Salin Rekening Jenis ke T.A
													{{ $store.getters["auth/TahunSelected"] }}
												</span>
											</v-card-title>
											<v-card-text>
												<v-alert type="warning">
													Menghindari duplikat proses salin, akan menghapus terlebih dahulu data Rekening Jenis T.A {{ $store.getters["auth/TahunSelected"] }}
												</v-alert>
												<v-select
		...
		data() {
			return {
				...
				rule_kode: [
					...
					value => value.length > 1 || "Kode Jenis minimal 2 angka",
				],
				rule_name: [
