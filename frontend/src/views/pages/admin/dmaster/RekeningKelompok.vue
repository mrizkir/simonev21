								...
								<v-dialog v-model="dialogcopyfrm" max-width="500px" persistent>
									...
									>
										<v-card>
											<v-card-title>
												<span class="headline">
													Salin Rekening Kelompok ke T.A
													{{ $store.getters["auth/TahunSelected"] }}
												</span>
											</v-card-title>
											<v-card-text>
												<v-alert type="warning">
													Menghindari duplikat proses salin, akan menghapus terlebih dahulu data Rekening Kelompok T.A {{ $store.getters["auth/TahunSelected"] }}
												</v-alert>
												<v-select
		...
		data() {
			return {
				...
				rule_kode: [
					...
					value => value.length > 0 || "Kode Kelompok minimal 1 angka",
				],
				rule_name: [
		...
		methods: {
			...
			salinrekening() {
				if (this.$refs.frmcopydata.validate()) {
					this.$ajax
						.post(
							"/dmaster/rekening/kelompok/salin",
							{
