		...
		<ModuleHeader>
			...
			<template v-slot:desc>
				<v-alert color="cyan" border="left" colored-border type="info">
					Daftar "rekening objek" sesuai dengan Keputusan Menteri Dalam Negeri
					...
					>
						<template v-slot:top>
							<v-toolbar flat color="white">
								...
								<v-tooltip bottom>
									...
									<span>Tambah Rekening Objek</span>
								</v-tooltip>
								...
								<v-dialog v-model="dialogcopyfrm" max-width="500px" persistent>
									...
									>
										<v-card>
											<v-card-title>
												<span class="headline">
													Salin Rekening Objek ke T.A
													{{ $store.getters["auth/TahunSelected"] }}
												</span>
											</v-card-title>
											<v-card-text>
												<v-alert type="warning">
													Menghindari duplikat proses salin, akan menghapus terlebih dahulu data Rekening Objek T.A {{ $store.getters["auth/TahunSelected"] }}
												</v-alert>
												<v-select
						...
						<template v-slot:item.actions="{ item }">
							<v-tooltip bottom>
								...
								<span>Detail Rekening Objek</span>
							</v-tooltip>
							<v-tooltip bottom>
								<template v-slot:activator="{ on, attrs }">
									<v-icon
							...
							<v-tooltip bottom>
								...
								<span>Hapus Rekening Objek</span>
							</v-tooltip>
						</template>
	...
	export default {
		...
		data() {
			return {
				...
				datatable: [],
				headers: [
					{ text: "KODE OBJEK", value: "kode_objek", width: 150 },
					{ text: "NAMA OBJEK", value: "ObyNm" },
				...
				//form rules
				rule_jenis: [
					value => !!value || "Mohon untuk di pilih rekening jenis !!!",
				],
				rule_kode: [
					value => !!value || "Mohon untuk di isi Kode Rekening Objek!!!",
					value =>
						/^[0-9]+$/.test(value) ||
			...
			save() {
				if (this.$refs.frmdata.validate()) {
					...
					if (this.editedIndex > -1) {
						this.$ajax
							.post(
								"/dmaster/rekening/objek/" + this.formdata.ObyID,
								{
									_method: "PUT",
				...
				this.$root.$confirm
					...
					.then(confirm => {
						if (confirm) {
							...
							this.$ajax
								.post(
									"/dmaster/rekening/objek/" + item.ObyID,
									{
										_method: "DELETE",
									},
									{
