  ...
  <RenjaMurniLayout :temporaryleftsidebar="true">
    <ModuleHeader>
      ...
      <template v-slot:name>
        REALISASI INDIKATOR SUB KEGIATAN MURNI
      </template>
  ...
  export default {
    ...
    mounted() {
      ...
      var OrgID_Selected = this.$store.getters["uiadmin/AtributeValueOfPage"](
        "realisasiindikatorsubkegiatanmurni",
        "OrgID_Selected"
      );
    ...
    methods: {
      ...
      async loaddata() {
        ...
        await this.$ajax
          .post(
            "/renjamurni/report/realisasiindikatorsubkegiatan",
            {
              tahun: this.$store.getters["auth/TahunSelected"],
      ...
      printtoexcel: async function() {
        ...
        await this.$ajax
          .post(
            "/renjamurni/report/realisasiindikatorsubkegiatan/printtoexcel",
            {
              tahun: this.$store.getters["auth/TahunSelected"],
              no_bulan: this.bulan_realisasi,
              SOrgID: this.SOrgID_Selected,
          ...
          .then(({ data }) => {
            ...
            link.href = url;
            link.setAttribute("download", "realisasi_indikator_sub_kegiatan_" + Date.now() + ".xlsx");
    ...
    watch: {
      OrgID_Selected(val) {
        var page = this.$store.getters["uiadmin/Page"]("realisasiindikatorsubkegiatanmurni");
