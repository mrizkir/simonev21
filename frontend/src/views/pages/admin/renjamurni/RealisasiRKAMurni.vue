	...
	export default {
		...
		methods: {
			...
			tambahRealisasi: async function() {
				await this.$ajax
					.get("/renja/rkamurni/bulanrealisasi/" + this.datauraian.RKARincID, {
						headers: {
							Authorization: this.$store.getters["auth/Token"],
						},
					})
			...
			save() {
				if (this.$refs.frmdata.validate()) {
					...
					if (this.editedIndex > -1) {
						this.$ajax
							.post(
								"/renja/rkamurni/updaterealisasi/" +
				...
				this.$root.$confirm
					...
					.then(confirm => {
						if (confirm) {
							...
							this.$ajax
								.post(
									"/renja/rkamurni/deleterealisasi/" + item.RKARealisasiRincID,
									{
										_method: "DELETE",
										pid: "datarealisasi",
									},
