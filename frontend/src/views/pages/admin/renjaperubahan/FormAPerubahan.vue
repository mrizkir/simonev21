	...
	export default {
		...
		mounted() {
			...
			if (this.formadetail) {
				...
				]("formaperubahan", "SOrgID_Selected");
		...
		methods: {
			...
			viewItem(item) {
				...
				this.initializeforma();
				this.$router.replace("/renjaperubahan/report/forma/" + item.RKAID);
			},
			...
			async printtoexcel() {
				...
				await this.$ajax
					.post(
						"/renjaperubahan/report/forma/printtoexcel",
						{
