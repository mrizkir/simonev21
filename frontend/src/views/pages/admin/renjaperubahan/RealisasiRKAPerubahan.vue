      ...
      <v-row class="mb-4" no-gutters>
        <v-col cols="12">
          <v-bottom-navigation color="purple lighten-1">
            <v-btn
              :to="{
                path: '/renjaperubahan/rka/uraian/' + this.RKARincID + '/edit',
              }"
            >
            ...
            <v-btn
              :to="{
                path: '/renjaperubahan/fotorealisasi/' + this.RKARincID,
              }"
            >
  ...
  export default {
    ...
    methods: {
      ...
      tambahRealisasi: async function() {
        await this.$ajax
          .get("/renja/rkaperubahan/bulanrealisasi/" + this.datauraian.RKARincID, {
            headers: {
              Authorization: this.$store.getters["auth/Token"],
            },
          })
      ...
      save() {
        if (this.$refs.frmdata.validate()) {
          ...
          if (this.editedIndex > -1) {
            this.$ajax
              .post(
                "/renja/rkaperubahan/updaterealisasi/" +
            ...
            this.$ajax
              .post(
                "/renja/rkaperubahan/saverealisasi",
                {
                  RKARincID: this.datauraian.RKARincID,
      ...
      deleteItem(item) {
        this.$root.$confirm
          .open(
            "Delete",
            "Apakah Anda ingin menghapus realisasi dengan ID " +
              item.RKARealisasiRincID +
              " ?",
            { color: "red" }
          )
          .then(confirm => {
            if (confirm) {
              ...
              this.$ajax
                .post(
                  "/renja/rkaperubahan/deleterealisasi/" + item.RKARealisasiRincID,
                  {
                    _method: "DELETE",
                    pid: "datarealisasi",
                  },
      ...
      exitrealisasi() {
        ...
        page.datarekening = {};
        this.$store.dispatch("uiadmin/updatePage", page);
        this.$router.push("/renjaperubahan/rka/uraian/" + page.datakegiatan.RKAID);
      },
    },
    watch: {
