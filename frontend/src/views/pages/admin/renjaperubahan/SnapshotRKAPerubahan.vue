<template>
  <RenjaPerubahanLayout :showrightsidebar="true">
    <ModuleHeader>
      <template v-slot:icon>
        mdi-source-fork
      </template>
      <template v-slot:name>
        SNAPSHOT RENJA PERUBAHAN
      </template>
      ...
      <template v-slot:desc>
        <v-alert color="cyan" border="left" colored-border type="info">
          Snapshot Rencana Kegiatan dan Anggaran (RKA) OPD / Unit Kerja APBD Perubahan <strong>bulan {{ nama_bulan }} T.A {{ $store.getters["auth/TahunSelected"] }}</strong>
        </v-alert>
      </template>
    </ModuleHeader>
    <v-container fluid>
  ...
  export default {
    ...
    methods: {
      ...
      loaddatakegiatanFirsttime: async function() {
        ...
        await this.$ajax
          .post(
            "/snapshot/rkaperubahan/loaddatakegiatanfirsttime",
            {
              tahun: this.$store.getters["auth/TahunSelected"],
        ...
        this.$root.$confirm
          ...
          .then(confirm => {
            if (confirm) {
              ...
              this.$ajax
                .post(
                  "/snapshot/rkaperubahan/" + tahun + this.$store.getters["uifront/getBulanRealisasi"],
                  {
                    _method: "DELETE",
                    pid: "all",                    
                    SOrgID: this.SOrgID_Selected,
      ...
      viewUraian(item) {
        ...
        if (page.datakegiatan.RKAID == "") {
          ...
          this.$router.push(
            "/renjaperubahan/snapshot/rka/uraian/" + page.datakegiatan.RKAID
          );
        } else {
          this.$root.$confirm
            .open(
