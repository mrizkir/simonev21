			...
			<v-row class="mb-4" no-gutters>
				<v-col cols="12">
					...
					>
						...
						<template v-slot:item.TargetKeuangan2="{ item }">
							{{ item.TargetKeuangan2 | formatUang }}
						</template>
		...
		methods: {
			...
			save() {
				if (this.$refs.frmdata.validate()) {					
					this.$ajax
						.post(
							"/renja/targetkinerjaperubahan/storekegiatan",
							{
			...
			viewUraian(item) {
				...
				if (page.datakegiatan.RKAID == "") {
					...
					this.$router.push(
						"/renjaperubahan/targetkinerja/" + page.datakegiatan.RKAID
					);
				} else {
					this.$root.$confirm
						.open(
