    ...
    <v-container v-if="Object.keys(datakegiatan).length > 1">
          ...
          <v-card>
            ...
            <v-card-text>
              <table class="table">
                <tbody>
                  <tr>
                    <td width="150" class="font-weight-bold">RKAID</td>
                    <td width="400">{{ datakegiatan.RKAID }}</td>
                    <td width="150" class="font-weight-bold">PAGU DANA</td>
                    <td width="400">
      ...
      <v-row class="mb-4" no-gutters>
        <v-col cols="12">
          <v-bottom-navigation color="purple lighten-1">
            ...
            <v-btn :to="{ path: '/renjaperubahan/rka/' + RKAID + '/edit' }">
              <span>Edit RKA</span>
      ...
      <v-row class="mb-4" no-gutters>
        <v-col cols="12">
          ...
          <v-dialog
            v-model="dialogtargetfisik"
            max-width="800px"
            v-if="dialogtargetfisik"
            persistent
          >
          ...
          <v-dialog
            v-model="dialogtargetanggarankas"
            max-width="800px"
            v-if="dialogtargetanggarankas"
            persistent
          >
            <v-form
              ref="frmtargetanggarankas"
              v-model="form_valid"
              lazy-validation
            >
              <v-card>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rule_totalfisik: [
          value =>
            new Number(value) == 100 ||
            "Total Target Fisik harus sama dengan 100% !!!",
        ],
          ...
          value => {
            if (typeof value !== "undefined" && value.length > 0) {
              ...
              return (
                ...
                "Total Target Anggaran Kas harus sama dengan Pagu Uraian (" +
                  Vue.filter("formatUang")(pagu_uraian) +
                  ")"
              );
            } else {
              return true;
    ...
    methods: {
      ...
      footersummary() {
        ...
        if (data.length > 0) {
          ...
          summary.persen_keuangan =
            totalrealiasi > 0 && totalpaguuraian > 0
              ? (totalrealiasi / totalpaguuraian) * 100
              : 0;
      ...
      viewItem(item) {
        ...
        if (page.datauraian.RKARincID == "") {
          ...
          this.$store.dispatch("uiadmin/updatePage", page);
          this.$router.push(
            "/renjaperubahan/rka/realisasi/" + page.datauraian.RKARincID
          );
        } else {
          this.$root.$confirm
            .open(
              "INFO",
              "Uraian lain sedang dibuka, jadi tidak bisa membuka Uraian ini",
              { color: "warning" }
            )
            .then(confirm => {
              if (confirm) {
      ...
      updateuraian: async function() {
        if (this.$refs.frmedituraian.validate()) {
          if (this.editedIndex > -1) {
            ...
            await this.$ajax
              .post(
                "/renja/rkaperubahan/updateuraian/" + this.formdata.RKARincID,
                {
                  _method: "PUT",
      ...
      savetargetfisik: async function() {
        if (this.$refs.frmtargetfisik.validate()) {
          ...
          if (this.editedIndex > -1) {
            await this.$ajax
              .post(
                "/renja/rkaperubahan/updatetargetfisik",
                {
                  _method: "PUT",
            ...
            await this.$ajax
              .post(
                "/renja/rkaperubahan/savetargetfisik",
                {
                  RKARincID: this.RKARincID_Selected,
                  RKAID: this.datakegiatan.RKAID,
                  bulan_fisik: this.formtarget.targetfisik,
      ...
      savetargetanggarankas: async function() {
        if (this.$refs.frmtargetanggarankas.validate()) {
          ...
          if (this.editedIndex > -1) {
            await this.$ajax
              .post(
                "/renja/rkaperubahan/updatetargetanggarankas",
                {
                  _method: "PUT",
                  RKARincID: this.RKARincID_Selected,
            ...
            await this.$ajax
              .post(
                "/renja/rkaperubahan/savetargetanggarankas",
                {
                  RKARincID: this.RKARincID_Selected,
                  RKAID: this.datakegiatan.RKAID,
                  bulan_anggaran: JSON.stringify(data_target),
      ...
      deleteItem(item) {
        this.$root.$confirm
          .open(
            "Delete",
            "Apakah Anda ingin menghapus data uraian RKA Perubahan dengan kode " +
              item.kode_uraian +
              " ?",
          ...
          .then(confirm => {
            if (confirm) {
              ...
              this.$ajax
                .post(
                  "/renja/rkaperubahan/" + item.RKARincID,
                  {
                    _method: "DELETE",
                    pid: "datauraian",
                  },
