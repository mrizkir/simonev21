		...
		<ModuleHeader>
			...
			<template v-slot:desc>
				<v-alert color="cyan" border="left" colored-border type="info">
					User dengan role BAPPELITBANG bertanggungjawab terhadap proses
				...
				await this.$ajax
					...
					.then(({ data }) => {
						...
						roles.forEach(element => {
							...
							} else if (element.name == "opd" || element.name == "pptk") {
								daftar_roles.push({
									text: element.name,
									disabled: false,
								});
			...
			save() {
				if (this.$refs.frmdata.validate()) {
					...
					if (this.editedIndex > -1) {
						this.$ajax
							.post(
								"/system/usersbapelitbang/" + this.editedItem.id,
								{
									_method: "PUT",
				...
				this.$root.$confirm
					...
					.then(confirm => {
						if (confirm) {
							...
							this.$ajax
								.post(
									"/system/usersbapelitbang/" + item.id,
									{
										_method: "DELETE",
									},
									{
		...
		computed: {
			formTitle() {
				return this.editedIndex === -1
					? "TAMBAH USER BAPPELITBANG"
					: "EDIT USER BAPPELITBANG";
			},
			...mapGetters("auth", {
