          ...
          >
            <template v-slot:top>
              <v-toolbar flat color="white">
                <v-toolbar-title>DAFTAR USERS DEWAN</v-toolbar-title>
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn
                      ...
                      class="ma-2"
                      :disabled="btnLoading"
                      @click.stop="showDialogTambahUserDewan"
                    >
                      <v-icon>mdi-plus</v-icon>
                    </v-btn>
                  </template>
                  <span>Tambah User Dewan</span>
                </v-tooltip>
            ...
            <template v-slot:item.active="{ item }">
              <div @click.stop>
                <v-switch
                  ...
                  :disabled="
                    btnLoading ||
                      !$store.getters['auth/can']('SYSTEM-USERS-DEWAN_UPDATE')
                  "
  ...
  export default {
    ...
    methods: {
      ...
      setUserActive(item, val) {
        ...
        this.$ajax
          .post(
            "/system/usersdewan/" + item.id + "/active",
            {
              _method: "PUT",
              active: next,
            },
      ...
      syncPermission() {
        this.$root.$confirm
          .open(
            ...
            "Sinkronisasi hanya untuk user dalam role dewan, bila user memiliki role lain akan terhapus permission-nya ?",
      ...
      showDialogTambahUserDewan: async function() {
        await this.$ajax
      ...
      save() {
        if (this.$refs.frmdata.validate()) {
          ...
          if (this.editedIndex > -1) {
            this.$ajax
              .post(
                "/system/usersdewan/" + this.editedItem.id,
                {
                  _method: "PUT",
