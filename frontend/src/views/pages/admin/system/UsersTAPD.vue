    ...
    <ModuleHeader>
      ...
      <template v-slot:desc>
        <v-alert color="cyan" border="left" colored-border type="info">
          User dengan role TAPD bertanggungjawab terhadap proses monitoring dan
          ...
          >
            <template v-slot:top>
              <v-toolbar flat color="white">
                <v-toolbar-title>DAFTAR USERS TAPD</v-toolbar-title>
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn
                      ...
                      class="ma-2"
                      :disabled="btnLoading"
                      @click.stop="showDialogTambahUserTAPD"
                    >
                      <v-icon>mdi-plus</v-icon>
                    </v-btn>
                  </template>
                  <span>Tambah User TAPD</span>
                </v-tooltip>
            ...
            <template v-slot:item.active="{ item }">
              <div @click.stop>
                <v-switch
                  ...
                  :disabled="
                    btnLoading ||
                      !$store.getters['auth/can']('SYSTEM-USERS-TAPD_UPDATE')
                  "
  ...
  export default {
    ...
    methods: {
      ...
      setUserActive(item, val) {
        ...
        this.$ajax
          .post(
            "/system/userstapd/" + item.id + "/active",
            {
              _method: "PUT",
              active: next,
            },
      ...
      syncPermission() {
        this.$root.$confirm
          .open(
            ...
            "Sinkronisasi hanya untuk user dalam role tapd, bila user memiliki role lain akan terhapus permission-nya ?",
      ...
      showDialogTambahUserTAPD: async function() {
        await this.$ajax
      ...
      save() {
        if (this.$refs.frmdata.validate()) {
          ...
          if (this.editedIndex > -1) {
            this.$ajax
              .post(
                "/system/userstapd/" + this.editedItem.id,
                {
                  _method: "PUT",
    ...
    computed: {
      formTitle() {
        return this.editedIndex === -1 ? "TAMBAH USER TAPD" : "EDIT USER TAPD";
      },
      ...mapGetters("auth", {
